...
def print_bill(request, order_id):
    ...
    # Use only root order items — child reorder items are already merged into root
    # when staff marks them ready. Looping all_orders would count them twice.
    combined_items = []
    combined_subtotal = Decimal('0')
    for item in root_order.items.select_related('menu_item').all():
        combined_items.append({
            'name': item.menu_item.name,
            'quantity': item.quantity,
            'unit_price': item.unit_price,
...
def live_order_data(request):
    ...
    # Only include active (non-completed) child orders in the display.
    # Completed children have already been merged into root.items, so including
    # them here would show the reorder items twice and inflate the total.
    active_children = [o for o in all_orders[1:] if o.status != 'completed']
    ...
    combined_parcel   = sum(o.parcel_charge for o in display_orders)
    combined_discount = sum(o.discount_amount for o in display_orders)
    combined_total    = combined_subtotal + combined_parcel - combined_discount
...
def staff_portal(request):
    ...
    for o in ready_orders:
        ...
        if has_reorders:
            combined_subtotal = sum(x.total_amount for x in active_linked)
...
def items_portal_add_item(request):
    ...
    try:
        ...
        # Save item data first without image so it always succeeds.
        # Then try saving the image — if Cloudinary fails the item still exists.
        image_file = request.FILES.get('image')
        item.image = None
        item.save()  # Always succeeds — no image yet
        ...
        if image_file:
            try:
                ...
                pass  # Item saved without image; Cloudinary may be misconfigured
    ...
    try:
        ...
        if image_file:
            try:
                ...
                pass  # Item data saved; image failed (Cloudinary misconfigured?)
        else:
            try:
                image_url = item.image.url if item.image else None
            except Exception:
