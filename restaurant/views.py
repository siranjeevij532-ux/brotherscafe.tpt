...
def place_order(request):
    ...
    try:
        ...
        if payment_method in ['offline', 'online']:
...
def order_status(request, order_id):
    ...
    # Build combined items list and total across active orders only
    combined_items = []
...
def bill_view(request, order_id):
    ...
    combined_subtotal = sum(o.subtotal for o in active_orders)
    ...
    combined_parcel   = sum(get_order_parcel(o) for o in active_orders)
    discount_percent  = root_order.discount_percent
    discount_amount   = sum(o.discount_amount for o in active_orders)
...
def print_bill(request, order_id):
    ...
    # Use only active orders for item totals — completed child reorders have already
    # been merged into the root order's items, so including them would double-count.
    active_orders = get_active_order_chain(root_order)
    combined_items = []
    combined_subtotal = Decimal('0')
    for o in active_orders:
...
def staff_portal(request):
    ...
    for o in ready_orders:
        ...
        if has_reorders:
            # Use root order's total_amount as the base (it already includes merged reorders).
            # Only add totals from children still active (not yet merged/completed).
            unmerged_active = [x for x in active_linked if x.id != root.id]
            combined_subtotal = root.total_amount + sum(x.total_amount for x in unmerged_active)
            ready_order_meta[o.id] = {
                'has_reorders': True,
                'combined_total': combined_subtotal,
                'root_order_number': root.order_number,
                'reorder_count': len(all_linked) - 1,
            }
        else:
            ready_order_meta[o.id] = {
                'has_reorders': False,
...
def update_order_status(request, order_id):
    try:
        with transaction.atomic():
            ...
            # Child reorders are merged into the root order when marked ready.
            if new_status == 'ready' and order.parent_order_id:
                root = order.parent_order
                while root.parent_order_id:
                    root = root.parent_order
                # Merge child order items into root order
                for oi in order.items.select_related('menu_item').all():
                    existing = root.items.filter(menu_item=oi.menu_item).first()
                    if existing:
                        existing.quantity += oi.quantity
                        existing.save()
                    else:
                        OrderItem.objects.create(
                            order=root, menu_item=oi.menu_item,
                            quantity=oi.quantity, unit_price=oi.unit_price,
                            notes=oi.notes or ''
                        )
                # Recalculate root order totals
                root.calculate_totals()
                Order.objects.filter(pk=root.pk).update(
                    subtotal=root.subtotal,
                    discount_amount=root.discount_amount,
                    total_amount=root.total_amount,
                )
                # Mark child order as completed so it disappears from all columns
                Order.objects.filter(pk=order.pk).update(
                    status='completed',
            ...
            if new_status == 'completed':
                order.completed_at = timezone.now()
                # ── Auto-merge any reorder children not yet merged (ready/preparing/accepted/pending)
                # This prevents double-billing when staff completes root without going through
                # the individual child "ready" step.
                if not order.parent_order_id:
                    unmerged_children = order.reorders.filter(
                        status__in=['pending', 'accepted', 'preparing', 'ready']
                    ).prefetch_related('items__menu_item')
                    for child in unmerged_children:
                        for oi in child.items.select_related('menu_item').all():
                            existing = order.items.filter(menu_item=oi.menu_item).first()
                            if existing:
                                existing.quantity += oi.quantity
                                existing.save()
                            else:
                                OrderItem.objects.create(
                                    order=order, menu_item=oi.menu_item,
                                    quantity=oi.quantity, unit_price=oi.unit_price,
                                    notes=oi.notes or ''
                                )
                        Order.objects.filter(pk=child.pk).update(
                            status='completed',
                            completed_at=timezone.now(),
                            payment_status='paid_offline',
                        )
...
def _build_excel(orders, title):
    ...
    headers = ['Order #','Type','Date','Time','Customer','Phone','Table',
               'Items','Subtotal (Rs)','Disc%','Disc (Rs)','Total (Rs)','Status','Payment','Cash Received','Change Due']
    for col, h in enumerate(headers, 1):
    ...
    for order in orders:
        ...
        ws.cell(row=row, column=15, value=float(order.cash_received or 0))
        ws.cell(row=row, column=16, value=float(order.change_amount or 0))
...
def items_portal_add_item(request):
    ...
    try:
        ...
        # Safely get image URL — Cloudinary may fail if not configured
        try:
